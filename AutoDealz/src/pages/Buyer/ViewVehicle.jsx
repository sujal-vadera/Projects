...
const ViewVehicle = () => {
  ...
  useEffect(() => {
    // Fetch current user from JSON Server (you can also rely on a global context if available)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const existingThread = allMessages.find(
        (msg) =>
          msg.vehicleId === vehicle.id &&
          msg.buyerEmail === currentUser.email &&
          msg.sellerEmail === vehicle.sellerEmail
      ...
      if (existingThread) {
        ...
        const updatedChat = [...existingThread.chat, newMessage];
        await axios.patch(`http://localhost:5000/messages/${existingThread.id}`, {
          chat: updatedChat,
        });
      } else {
        // Create new thread
